...
main_menue_func() # It display the main menue option on to the screen
{
    tput clear
    tput cup 5 10
    ...
    error_flag=0
    main_read_options_func
}
bill_info_func() # It will store the bill information of the client against it id
{
    tput clear
    tput cup 5 10
...
customer_info_func() # It will read the customer data
{
    tput clear
    tput cup 5 10
...
report_menue_func() # It will create the report against the client id
{
    #Here you need to write the code for report menue
    tput clear
    tput cup 5 10 
    echo "UNIX Library - ${BOLD}REPORT MENU${NORMAL}"
    tput cup 7 20 ;   echo "Enter Client ID for Report:  "
    ...
    echo "Client information with bill details against the Input ID"
    echo "__________________________________________________________"

    if(($line_num > 0))
    then
        sed -n "${line_num}p" results.txt | awk '{print $1}'
